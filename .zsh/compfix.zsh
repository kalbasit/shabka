# Taken from https://github.com/robbyrussell/oh-my-zsh/blob/master/lib/compfix.zsh
#
...
function handle_completion_insecurities() {
  ...
  if (( ! ${#insecure_dirs} )); then
      print "[zsh] No insecure completion-dependent directories detected."
      return
  fi
  ...
  ls -ld "${(@)insecure_dirs}"
  print "[zsh] For safety, completions will be disabled until you manually fix all"
  print "[zsh] insecure directory permissions and ownership and restart zsh."
  print "[zsh] See the above list for directories with group or other writability.\n"
  ...
  if (( ${#zcompdump_files} )); then
    ...
    print "[zsh] Insecure completion caches also detected:"
    ls -l "${(@)zcompdump_files}"

    # For safety, move rather than permanently remove such files.
    print "[zsh] Moving to \"${ZSH_ZCOMPDUMP_BAD_DIR}/\"...\n"
